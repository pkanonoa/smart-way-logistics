...
export default function VehiclesPage({ embedded = false }) {
  const { user } = useAuth();
      ...
      <div className={embedded ? '' : 'max-w-7xl mx-auto px-6 py-8'}>
        ...
        {embedded && canEdit(user?.role) && (
          <div className="flex justify-end mb-4">
            <button onClick={openCreate} className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-400 text-white rounded-xl px-4 py-2 text-sm font-semibold transition-all">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
              Add Vehicle
