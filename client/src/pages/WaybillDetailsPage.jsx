...
export default function WaybillDetailsPage() {
  ...
  const fetchWaybill = async () => {
    ...
    try {
      ...
      setWaybill(data);
      setInvoiceNumberInput(data.invoice_number || '');
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between items-center pb-2 border-b border-slate-800">
              ...
              <span className="text-white font-mono font-medium">{waybill.invoice_number || '—'}</span>
            </div>
            {waybill.eway_bill_number ? (
              ...
              <div className="text-center py-4 bg-cyan-500/10 rounded-xl border border-cyan-500/30 border-dashed mt-2">
                <p className="text-cyan-400 text-xs font-semibold">E-Way Bill Required — Not Added</p>
                <p className="text-slate-400 text-[11px] mt-0.5">Shipments over ₹50,000 require an E-Way Bill</p>
              </div>
            ) : (
              <div className="flex justify-between items-center">
                <span className="text-slate-400">E-Way Bill</span>
                <span className="text-slate-500 text-xs">Not added</span>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Invoice & E-Way Bill Modal */}
      {showEWayModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">Update Invoice & E-Way Bill</h3>
            ...
            <form onSubmit={handleUpdateEWayBill} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Invoice Number</label>
                <input
                  type="text"
